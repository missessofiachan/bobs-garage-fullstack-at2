				...
				{staff.map((m) => {
					...
					const photoSrc = m.photoUrl
						? m.photoUrl.startsWith("http")
							? m.photoUrl
							: m.photoUrl.startsWith("/")
																					? `${getImageBaseUrl()}${m.photoUrl}`
													: `${getImageBaseUrl()}/${m.photoUrl}`
						: undefined;
						...
						<Col key={m.id}>
							<Card>
								{photoSrc ? (
									...
									<Card.Img variant="top" src={SVG_PLACEHOLDER} alt="placeholder" />
								)}
								<Card.Body>
									<Card.Title className="mb-1">{m.name}</Card.Title>
									{m.role && <div className="text-muted mb-2">{m.role}</div>}
									{m.bio && <Card.Text style={{ whiteSpace: "pre-wrap" }}>{m.bio}</Card.Text>}
								</Card.Body>
							</Card>
						</Col>
					);
