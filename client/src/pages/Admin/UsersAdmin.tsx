...
export default function UsersAdmin() {
	...
	const onCreate = async (e: React.FormEvent) => {
		e.preventDefault();
		await createUser.mutateAsync({ email, password, role: 'user', active: true });
	...
	return (
		<div>
			...
			<Table striped bordered hover size="sm">
				<thead><tr><th>ID</th><th>Email</th><th>Role</th><th>Active</th><th></th></tr></thead>
				<tbody>
					{(data ?? []).map(u => (
						<tr key={u.id}>
							<td>{u.id}</td>
							...
							<td>
								<Button variant="danger" size="sm" onClick={() => deleteUser.mutate(u.id)}>Delete</Button>
							</td>
						</tr>
					))}
				</tbody>
