...
export default function Autocomplete({
  ...
  const suggestions = useMemo(() => {
    if (!services || services.length === 0 || query.length === 0) {
      return [];
    }
  ...
  }, [highlightedIndex, suggestions, value]);

  useEffect(() => {
    const trimmedValue = value.trim();
    const shouldOpen = trimmedValue.length > 0 && suggestions.length > 0;
    if (
  ...
  useEffect(() => {
    ...
    const highlightedElement = items[highlightedIndex] as HTMLElement | undefined;
          ...
          {suggestions.map((service, index) => (
            <div
              ...
              style={{
                ...
                borderBottom:
                  index < suggestions.length - 1 ? '1px solid var(--bs-border-color)' : 'none',
              }}
            >
