 ...
 */
export function createResourceHooks<DTO extends Record<string, unknown>, CreateDTO = Partial<DTO>, UpdateDTO = Partial<DTO>>(config: ResourceQueryConfig) {
  const queryKey = {
    ...
    useList: (options?: any) => {
      return useQuery<DTO[], unknown, DTO[], any>({
        ...
        queryFn: async (): Promise<DTO[]> => {
          const { data } = await api.get<DTO[]>(config.basePath);
          return data;
        },
    ...
    useUpdate: (options?: any) => {
      ...
      return useMutation({
        mutationFn: async ({
          id,
          ...payload
        }: UpdateDTO & { id: number }): Promise<DTO> => {
