...
function createMockJwt(payload: Record<string, unknown>): string {
  ...
  const payloadPart = btoa(JSON.stringify(payload)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
...
describe('decodeJwt', () => {
  ...
  it('should handle payload with nested objects', () => {
    ...
    const decoded = decodeJwt<{ sub: number; metadata: { role: string; permissions: string[] } }>(token);
