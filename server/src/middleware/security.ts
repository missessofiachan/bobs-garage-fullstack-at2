 ...
 * @purpose Security middleware configuration (Helmet, CORS, cookie parser)
 * @version 1.0.0
 */
...
export function applySecurity(app: Express) {
	...
	if (origins.length === 0) {
		...
		if (env.NODE_ENV !== "production") {
			...
			console.warn("No CORS_ORIGINS provided; falling back to http://localhost:5173");
		}
	...
	const corsOptions = {
		origin: (origin: unknown, callback: (err: Error | null, allow?: boolean) => void) => {
			...
			if (typeof origin === "string" && origins.includes(origin)) return callback(null, true);
