...
export function createApp(): Application {
  ...
  app.get('/db-status', async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ status: 'disconnected', error: (err as Error).message });
    }
  });
  ...
  app.use(
    (
      ...
      winstonLogger.error(`Unhandled error in request pipeline: ${err instanceof Error ? err.message : String(err)}`);
