...
describe('auth.service', () => {
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should throw error for invalid refresh token', async () => {
      ...
      await expect(refreshAccessToken(refreshToken)).rejects.toThrow('Invalid or expired refresh token');
      expect(mockVerifyRefreshToken).toHaveBeenCalledWith(refreshToken);
      expect(logger.warn).toHaveBeenCalledWith({ err: error }, 'Failed to verify refresh token');
    });
  });
});
