...
describe("API rate limiting", () => {
	...
	test("authenticated users get separate rate limit buckets", async () => {
		...
		for (let i = 0; i < user1Attempts; i++) {
			const res = await agent1.get("/api/health").set("Authorization", `Bearer ${token1}`);
			if (res.status === 200) {
				user1Success++;
			}
			await new Promise((r) => setTimeout(r, 1));
		...
		for (let i = 0; i < user2Attempts; i++) {
			const res = await agent2.get("/api/health").set("Authorization", `Bearer ${token2}`);
			if (res.status === 200) {
				user2Success++;
			}
			await new Promise((r) => setTimeout(r, 1));
	...
	test("authenticated users get higher rate limits than unauthenticated", async () => {
		...
		for (let i = 0; i < authenticatedAttempts; i++) {
			const res = await agent.get("/api/health").set("Authorization", `Bearer ${token}`);
			if (res.status === 200) {
