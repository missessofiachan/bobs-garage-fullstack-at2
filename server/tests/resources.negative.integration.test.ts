...
describe("Additional negative validations for services/staff", () => {
	...
	test("services: invalid id param on get/update/delete -> 400", async () => {
		...
		const d = await agent.delete("/api/services/abc").set("Authorization", `Bearer ${adminAccess}`);
	...
	test("services: update with invalid payload -> 400", async () => {
		...
		// cleanup
		const d = await agent.delete(`/api/services/${id}`).set("Authorization", `Bearer ${adminAccess}`);
	...
	test("staff: invalid id param on get/update/delete -> 400", async () => {
		...
		const d = await agent.delete("/api/staff/abc").set("Authorization", `Bearer ${adminAccess}`);
		expect([400, 404]).toContain(d.status);
	});
	...
	test("staff: create/update invalid payload -> 400", async () => {
		...
		// create ok
		const c = await agent.post("/api/staff").set("Authorization", `Bearer ${adminAccess}`).send({
			name: "Alice",
			role: "Mechanic",
			bio: "pro",
		...
		// cleanup
		const d = await agent.delete(`/api/staff/${id}`).set("Authorization", `Bearer ${adminAccess}`);
		expect([200, 204]).toContain(d.status);
	});
});
